...
<body>
    ...
    <section class='chatBox'>
        <h4>Чаты</h4><br>
        <div class='couple_widgets_column assistsBox'>
            <?php 
            
            $login = $_SESSION['user'];

            $chats = array();
            $chats_sql = mysqli_query($link, "SELECT * FROM `chat` WHERE `by`='{$login}' OR `to`='{$login}'");
            if (mysqli_num_rows($chats_sql) > 0){
                while($result = mysqli_fetch_array($chats_sql)){
                    if ($login != $result['by']){
                        $assist = $result['by'];
                    } else {
                        $assist = $result['to'];
                    }
                    if (!in_array($assist, $chats)){
                        $chats[] = $assist;
                    }
                }
                $elem = "";
                for ($i = 0; $i < count($chats); $i++){
                    $assist = $chats[$i];
                    $the_chat = mysqli_query($link, "SELECT * FROM `chat` WHERE (`by`='{$login}' AND `to`='{$assist}') OR (`to`='{$login}' AND `by`='{$assist}') ORDER BY `id` DESC LIMIT 1");
                    $lastmess = "";
                    while ($t_c = mysqli_fetch_array($the_chat)){
                        if ($t_c['by'] == $login){
                            $lastmess .= "Вы: ";
                        }
                        if ($t_c['img'] != '0'){
                    ...
                    $user = mysqli_query($link, "SELECT `ava` FROM `users` WHERE `login`='{$assist}'");
                    while ($us = mysqli_fetch_assoc($user)){
                        $ava = $us['ava'];
                    }

                    $new_messages = mysqli_query($link, "SELECT * FROM `chat` WHERE `to`='{$login}' AND `by`='{$assist}' AND `read` = 0 ORDER BY `id`");
                    $news_count = mysqli_num_rows($new_messages);

                    if ($news_count > 0){
                        $news_marker = "<div class='newMess_marker'>{$news_count}</div>";
                    } else {
                        $news_marker = "";
                    }

                    $elem .= "
                        <a href='/chat.php?assist={$assist}' class='assistWidget'>
                            <div class='couple_widgets' style='margin-top:10px;margin-left:15px'>
                                <img src='/uploads/{$ava}' class='profile_ava'>
                                <h4 style='margin-left:15px;margin-top: 15px;'>{$assist}</h3>
                                {$news_marker}
                            </div>
                            <p class='lastmess'>{$lastmess}</p>
                        </a>";
                } 
                ...
                echo "<p class='pseudo_info' style='text-align: center'>Нет чатов</p>"; 
            }
    ...
    <script type='text/javascript'>
        localStorage.removeItem('newMess_notif');
    </script>
